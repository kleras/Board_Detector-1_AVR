...
uint8_t sim900_setup(const uint8_t isWaitingDevice)
{
	...
	#warning Modem setup should include AT+CMGDA="DEL ALL" + set default config, default settings etc.
	...
	/*
	//del sms
	sim900_send_cmd_wait_reply(1,(const uint8_t*)PSTR("AT+CMGDA=\"DEL ALL\"\r"), (const
	uint8_t*)RESPON_OK, 500000, 0, NULL);
	_delay_ms(10);
	*/
	...
	if (respons)
	{
		...
		uint8_t*)RESPON_OK, 500000, 0, NULL)))
		{
			...
			(const uint8_t*)RESPON_OK, 500000, 0, NULL)))
			{
				...
				// Neveikia be SIM -  +CMS ERROR: SIM not inserted
				_delay_ms(10);
...
void modem_control_pins_init() // intern
{
	// Outputs.
...
uint8_t modem_power_on()
{
	...
	modem_control_pins_init(); // Ar ne cia?
	
	/*
	
	#warning RING: O jei paskambins arba sms?:D Reikes pakeist kai bus status pin.
	if(bit_is_clear(RING_INT1_PIN_R, RING_INT1_PIN)) // O jei paskambins arba sms?:D
	{
		return 1; // Modem already on
	}	
